...
const CreatorsPage = () => {
  ...
  const handleDownloadExcel = () => {
    ...
    const selectedData = creators.filter((creator) =>
      selectedRows.includes(creator.creator_id)
    );
  ...
  const filteredCreators = creators.filter((creator) => {
    return Object.keys(filters).every((key) => {
      return (
        filters[key] === "" ||
        String(creator[key]).toLowerCase().includes(filters[key].toLowerCase())
      );
    });
  });
    ...
    <>
      ...
      <Box bg={cardBg} p={4} mb={4} borderRadius="md" shadow="md">
        ...
        >
          ...
          <Popover>
            <PopoverTrigger>
              <Button leftIcon={<FiEye />} size="sm" variant="outline">
                Columns
              </Button>
            </PopoverTrigger>
            <PopoverContent>
              ...
              <PopoverBody>
                <CheckboxGroup
                  value={Object.keys(tempSelectedColumns).filter(
                    (col) => tempSelectedColumns[col]
                  )}
                >
            ...
            <Thead bg={tableHeaderBg}>
              <Tr>
                <Th>
                  <Checkbox
                    isChecked={
                      selectedRows.length === filteredCreators.length &&
            ...
            <Tbody>
              {filteredCreators.map((creator) => (
                <Tr key={creator.creator_id}>
                  ...
                  {Object.keys(selectedColumns).map(
                    (col) =>
                      selectedColumns[col] && <Td key={col}>{creator[col]}</Td>
                  )}
                  {hasEditPermission() && (
                    <Td>
                      <Button
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme="teal" onClick={handleSave}>
              {isEdit ? "Update Creator" : "Add Creator"}
            </Button>
          </ModalFooter>
        </ModalContent>
      </Modal>

...
export default function CampaignPage() {
  ...
  const [formData, setFormData] = useState({ name: '', description: '', start_date: '', end_date: '', budget: '', brand: '', status: '' });
  ...
  const openModal = (campaign = null) => {
    if (campaign) {
      ...
      setFormData({ name: '', description: '', start_date: '', end_date: '', budget: '', brand: '', status: '' });
      setIsEdit(false);
    }
    onOpen();
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit) {
        ...
        toast({ title: 'Campaign updated successfully!', status: 'success', duration: 3000 });
      } else {
        await createCampaign(formData, token);
        toast({ title: 'Campaign created successfully!', status: 'success', duration: 3000 });
      }
      onClose();
      loadCampaigns(token);
    } catch (err) {
      console.error('Failed to save campaign:', err);
      toast({ title: 'Failed to save campaign.', status: 'error', duration: 3000 });
    }
  };
  ...
  const handleDelete = async (campaignId) => {
    ...
    try {
      ...
      toast({ title: 'Campaign deleted successfully!', status: 'success', duration: 3000 });
      loadCampaigns(token);
    } catch (err) {
      console.error('Failed to delete campaign:', err);
      toast({ title: 'Failed to delete campaign.', status: 'error', duration: 3000 });
    }
  };
  ...
  const filteredCampaigns = campaigns.filter((campaign) => {
    return (
      ...
      (!filters.brand || campaign.brand.toLowerCase().includes(filters.brand.toLowerCase())) &&
  ...
  return (
    <SidebarWithHeader>
      ...
      <Collapse in={isFilterOpen} animateOpacity>
        <Box bg={cardBg} p={4} mb={6} borderRadius="md" shadow="sm">
          ...
          <Flex justify="flex-end">
            <Button onClick={clearFilters} colorScheme="red" variant="outline" size="sm">
              Clear Filters
            </Button>
          </Flex>
        </Box>
      ...
      <Modal isOpen={isOpen} onClose={onClose} size={modalSize}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{isEdit ? 'Edit Campaign' : 'Create New Campaign'}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Input
              placeholder="Campaign Name"
